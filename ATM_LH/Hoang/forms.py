...
class SigninForm(forms.ModelForm):
    account_no = forms.CharField(widget=forms.TextInput(
        attrs={'placeholder': 'Số tài khoản'}
    ), required=True, max_length=20)
    password = forms.CharField(widget=forms.PasswordInput(
        attrs={'placeholder': 'PIN'}
...
class RegistrationForm(forms.ModelForm):
    ...
    class Meta:
        model = Customer
        fields = ['full_name', 'card_type', 'card_no', 'gender', 'birthday', 'phone_number', 'email', 'address',
                  'branch_id']

    def clean_card_no(self):
        card_number = self.cleaned_data['card_no']
